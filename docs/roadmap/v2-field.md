...
> 🔮 **v1 범위 밖**. 이 문서는 v2 작업 시작 시점에 분해되어 본 문서들(03/04/05/06/07)의 해당 섹션으로 흩어진다.
> 지금은 "v1 에서 의도적으로 뺀 것" 의 단일 보관소.

## 배경

v1 은 관리자 1명이 두 엑셀을 업로드 / drift 검증 / 알림을 보는 데 집중. 현장직원이 모바일 PWA 로 본인 작업을 관리하는 영역은 v2 에 분리한다.

## v2 에서 추가할 것 (체크리스트)

### 데이터 모델 ([[../03-data-model]] 갱신 필요)
